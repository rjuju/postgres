						   ...
						   List *partitioned_rels)
{
	...
	/*
	 * Generate sorted Append pahts if relation is partitioned table and has
	 * natural ordering
	 */
	if (rel->part_sorted && has_useful_pathkeys(root, rel) &&
		 root->query_pathkeys)
	{
		List *pathkeys = generate_pathkeys_for_partitioned_table(root, rel);
		List *ordered_partitions = NIL;
		int i;
		PathKey *first;
		...
		{
			if(first->pk_strategy == BTLessStrategyNumber)
				ordered_partitions = lappend(ordered_partitions, rel->part_rels[i]);
			else
				ordered_partitions = lcons(rel->part_rels[i], ordered_partitions);
		}

		Assert(pathkeys_contained_in(root->query_pathkeys, pathkeys));

		first = linitial(pathkeys);
		foreach(lcr, ordered_partitions)
		{
		...
		if (startup_subpaths)
			add_path(rel, (Path *) create_append_path(root, rel,
						...
						NULL, 0, false, NIL, -1));
		if (startup_neq_total && total_subpaths)
			add_path(rel, (Path *) create_append_path(root, rel,
	...
	/* Generate standard MergeAppend paths */
